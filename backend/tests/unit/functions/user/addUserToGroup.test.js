...
const { addUserToGroup } = require('../../../../src/functions/user/addUserToGroup/service');
...
describe('addUserToGroup', () => {
    ...
    it('should add user to group successfully', async () => {
        ...
        const result = await addUserToGroup({ email: 'test@example.com', groupName: 'Recruiters' });

        expect(result.status).toBe('success');
        expect(result.message).toBe('User added to group successfully');
        expect(__mocks__.sendMock).toHaveBeenCalledWith(expect.any(AdminAddUserToGroupCommand));
    });

    it('should throw an error if email or groupName is missing', async () => {
        await expect(addUserToGroup({ email: '', groupName: 'Recruiters' })).rejects.toThrow('Email and groupName are required');
    });
    ...
    it('should throw an error if adding user to group fails', async () => {
        ...
        await expect(addUserToGroup({ email: 'test@example.com', groupName: 'Recruiters' })).rejects.toThrow('Failed to add user to group');
    });
});
